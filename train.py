...
for epoch in range(63, 500):
    ...
    for i, data in tqdm(enumerate(dataloader, 0), desc=f"epoch {epoch} train", total=len(dataloader)):
        ...
        if train_count != 0 and train_count % 8 == 0:
            ...
            tqdm.write(f"{train_count} {float(train_dis_avg) / 8.0}")
    ...
    model.eval()
    score = []
    for j, data in tqdm(enumerate(testdataloader, 0),desc=f"epoch {epoch} test", total=len(testdataloader)):
        img_fr, choose_fr, cloud_fr, r_fr, t_fr, img_to, choose_to, cloud_to, r_to, t_to, mesh, anchor, scale, cate = data
        img_fr, choose_fr, cloud_fr, r_fr, t_fr, img_to, choose_to, cloud_to, r_to, t_to, mesh, anchor, scale, cate = Variable(
            img_fr).cuda(), \
            Variable(choose_fr).cuda(), \
